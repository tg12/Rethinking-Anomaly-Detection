...
def train(model, g, args):
    ...
    best_f1, final_trec, final_tpre, final_tmf1, final_tauc = (
        0.0,
        0.0,
        0.0,
        0.0,
    ...
    for e in range(args.epoch):
        ...
        loss = F.cross_entropy(
            logits[train_mask], labels[train_mask], weight=torch.tensor([1.0, weight])
        )
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if best_f1 < f1:
            ...
            final_tmf1 = tmf1
            final_tauc = tauc
        print(f"Epoch {e}, loss: {loss:.4f}, val mf1: {f1:.4f}, (best {best_f1:.4f})")
...
if __name__ == "__main__":
    ...
    if args.run == 1:
        ...
        print(
            f"MF1-mean: {100 * np.mean(final_mf1s):.2f}, MF1-std: {100 * np.std(final_mf1s):.2f}, AUC-mean: {100 * np.mean(final_aucs):.2f}, AUC-std: {100 * np.std(final_aucs):.2f}"
        )
